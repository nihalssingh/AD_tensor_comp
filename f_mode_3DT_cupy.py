...
from derv import derv
from derv import j_derv
import timeit
import autograd
import torch as pt
...
class f_node:
    ...
    def einsum(s1, s2, s3, s4, a, b):
        ...
        prod = cp.einsum(s_prod, a.val, b.val, optimize=True)
        # prod = contract(s_prod, a.val, b.val)
        # b.der = 0 tensor
        if cp.any(b.der) != True:
            der = cp.einsum(s_dera, b.val, a.der, optimize=True)
            # der = contract(s_dera, b.val, a.der)
        elif cp.any(a.der) != True:
            der = cp.einsum(s_derb, a.val, b.der, optimize=True)
            # der = contract(s_derb, a.val, b.der)
        else:
            der = cp.einsum(s_dera, b.val, a.der, optimize=True) + cp.einsum(s_derb, a.val, b.der, optimize=True)
            # der = contract(s_dera, b.val, a.der) + contract(s_derb, a.val, b.der)
        # t1 = cp.einsum(s_dera, b.val, a.der)
        # t2 = cp.einsum(s_derb, a.val, b.der)
    ...
    def sin(s1, s2, a):
        ...
        val = cp.sin(a.val)
        der = cp.einsum(s_der, cp.cos(a.val), a.der)
        # der = contract(s_der, cp.cos(a.val), a.der)
        v = f_node(val, der)
        return v
    ...
    def cos(s1, s2, a):
        ...
        s_der = s_template.format(s1, s1+s2, s1+s2)
        val = cp.cos(a.val)
        der = cp.einsum(s_der, -cp.sin(a.val), a.der)
        v = f_node(val, der)
        return v
...
for iters in range (1,6):
    ...
    A = cp.random.rand(1+iters*1, 1+iters*1, 2)
    B = cp.random.rand(1+iters*1, 1+iters*1, 2)
    C = cp.random.rand(3, 1+iters*1, 1+iters*1)
    
    # AA = pt.rand(1+iters*1, 1+iters*1, 2)
    # BB = pt.rand(1+iters*1, 1+iters*1, 2)
    # CC = pt.rand(3, 1+iters*1, 1+iters*1)
    
    # A = cp.random.rand(2, 4, 3, 3)
    # B = cp.random.rand(2, 4, 3, 3)
    # C = cp.random.rand(2, 4, 3, 3)
    ...
    for t in range(1, 10):
        ...
        # Calculating dXi/dXi for all icputs to be used later
        d_I = []
        ...
        # Looping over all icputs variables Xj to calculate dF/dXj
        for i in range(0, n_icp):
            x = []
            shape = cp.zeros([])
            s4 = ''
            
            #Initialising the icput nodes with derivative values: dXj/dXi o=for all i and j
            for j in range(0, n_icp):
                # s4 and dXi/dXi calculation 
                if i==j:
                    x.append(f_node(I[i], d_I[i]))
                    
                    #Calculating s4
                    for l in range(0, len(cp.shape(I[i]))):
                        s4 = s4 + chr(ord('z')-l)
                # dXj/dXi
                else:
                    shape = cp.shape(I[j])
                    shape = cp.append(shape, cp.shape(I[i]))
                    
                    x.append(f_node(I[j], cp.zeros(shape.tolist())))
            ...
            if len(A.shape) == 4:
                f = f_node.einsum('ijkl','mnop','ijklmnop', s4, x[0], x[2]) + f_node.einsum('ijkl','mnop','ijklmnop', s4, x[1], x[2])
            if len(A.shape) == 3:
                # f = f_node.einsum('ijk','lmn','ijklmn', s4, x[0], x[2]) + f_node.einsum('ijk','lmn','ijklmn', s4, x[1], x[2])
                f = f_node.einsum('ijk','lmn','ijklmn', s4, x[0], x[2])
            # f = f_node.sin('ijk', s4, x[0]) + f_node.cos('ijk', s4, x[1])
            
            f_der.append(f.der)
            # print(cp.shape(f_der[i]))
    ...
    # plt.grid()
    # plt.scatter(cp.prod(f_val.shape), g_ot[iters-1], color='green')
    # plt.scatter(cp.prod(f_val.shape), g_at[iters-1], color='blue')
    # plt.scatter(cp.prod(f_val.shape), g_pt[iters-1], color='red')
